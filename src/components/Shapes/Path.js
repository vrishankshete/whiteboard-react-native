...
export default class Path extends React.Component{
    ...
    renderShape(){
        ...
        if(showToolTip){
            return (
                <G> 
                    <Svg.Path fill="none" strokeWidth={style.strokeWidth} stroke={style.stroke} strokeDasharray={style.strokeDasharray} d={d}/>
        ...
        if(showOverlay){
            ...
            return (
                <G> 
                    {this.state.mouseOverMe?<Svg.Path fill="none" strokeWidth={styleOverlay.strokeWidth} stroke={styleOverlay.stroke} strokeLinecap={'round'} strokeOpacity={0.5} strokeDasharray={styleOverlay.strokeDasharray} d={d}/>:''}
                    <Svg.Path fill="none" strokeWidth={style.strokeWidth} stroke={style.stroke} strokeDasharray={style.strokeDasharray} d={d}/>
                    <Svg.Path onPressIn={()=>{this.mouseEntered()}} onPressOut={()=>{this.mouseOut(); this.props.removeDrawing(drawingId);}}
        ...
        return(<Svg.Path fill="none" strokeWidth={style.strokeWidth} stroke={style.stroke} strokeDasharray={style.strokeDasharray} d={d}/>);
    }
